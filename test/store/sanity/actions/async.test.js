...
describe('Sanity Store: Async Actions', () => {
  ...
  describe('isCoreApiPasswordValid', () => {
    it('returns password valid & core running actions if password is valid', () => {
      ...
      const corePasswordProtectedReadUrl =
        'https://core.blockstack.org/v1/wallet/payment_address'
      const coreApiPassword = 'secretdonttell'

      return store
        .dispatch(
          SanityActions.isCoreApiPasswordValid(
    ...
    it('returns password not valid action if password is wrong', () => {
      nock('http://localhost:6270')
        .get('/v1/wallet/payment_address')
        .reply(403, {}, { 'Content-Type': 'application/json' })

      const store = mockStore({})

      const corePasswordProtectedReadUrl =
        'http://localhost:6270/v1/wallet/payment_address'
      ...
      return store
        ...
        .then(() => {
          const expectedActions = [{ type: CORE_API_PASSWORD_NOT_VALID }]
          assert.deepEqual(store.getActions(), expectedActions)
        })
    })
  ...
  describe('isCoreRunning', () => {
    it('returns true if core node status is alive', () => {
      ...
      return store
        .dispatch(SanityActions.isCoreRunning(corePingUrl))
        .then(() => {
